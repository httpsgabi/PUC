...
namespace Exercício004
{
    internal class Program
    {
        static void ExibePesos(int[] pesosAnimais, int[,] maiorEmenorPeso, string[] nomesAnimais)
        {
            ...
            StreamWriter arquivo3 = new StreamWriter("arquivo-03.txt", false, Encoding.UTF8);
            for (int i = 0; i < pesosAnimais.Length; i++)
            {
                for (int j = 0; j < maiorEmenorPeso.GetLength(0); j++)
                {
                    ...
                    {
                        arquivo3.WriteLine($"O animal {nomesAnimais[i]}, possui peso {pesosAnimais[i]}, e está presente no intervalo de {intervalo1} até {intervalo2}");
                    }
                }
